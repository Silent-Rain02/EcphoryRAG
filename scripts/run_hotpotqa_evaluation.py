#!/usr/bin/env python
"""
HotpotQA Evaluation Script for EcphoryRAG

This script demonstrates how to use the Evaluator class to evaluate 
EcphoryRAG performance on the HotpotQA multi-hop question answering dataset,
...
def get_processed_hotpotqa_data(processed_dir: str = "data/processed_hotpotqa") -> str:
    """
    Get the path to the processed HotpotQA dataset.
    ...
    dev_path = os.path.join(project_root, processed_dir, "train_processed.jsonl")
    
    # Check if file exists
    if os.path.exists(dev_path):
        logger.info(f"Processed HotpotQA dataset found at {dev_path}")
        return dev_path
    else:
        logger.error(f"Processed HotpotQA dataset not found at {dev_path}")
        logger.info(
            "Please process the raw HotpotQA dataset first using the process_hotpotqa_data.py script:"
            "\npython scripts/process_hotpotqa_data.py --raw_data_file data/hotpotqa/hotpotqa_dev.json "
...
def parse_args():
    ...
    parser = argparse.ArgumentParser(
        description="Evaluate EcphoryRAG on processed HotpotQA dataset",
    ...
    parser.add_argument("--top-k-final-values", type=str, default="1,3,5,10,20,40,80,120",
...
def main():
    """Run HotpotQA evaluation on EcphoryRAG using processed dataset."""
    args = parse_args()
    ...
    if args.data_path:
        ...
        data_path = get_processed_hotpotqa_data(args.processed_dir)
    ...
    if args.test_subset:
        ...
        logger.info(f"Using full HotpotQA dataset at: {data_path}")
    
    # Initialize processed HotpotQA dataset loader
    logger.info(f"Loading processed HotpotQA dataset from {data_path}")
    dataset_loader = ProcessedHotpotQADatasetLoader(data_path)
    ...
    try:
        ...
        try:
            ...
            print_results_table(results, title="HotpotQA Evaluation Results")
        except ImportError:
